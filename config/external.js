...
const externalConfigs = {
  ...
  'axios': {
    name: 'axios',
    resources: isDevelopment => getDistVendorResources('axios', isDevelopment),
  },
  'jquery': {
    name: 'jQuery',
  ...
  'tinymce': {
    name: 'tinymce',
    resources: () => ({
      scripts: [path.join(__dirname, '../node_modules/tinymce/tinymce.min.js')],
      styles: [path.join(__dirname, '../node_modules/tinymce/skins/ui/oxide/skin.min.css')],
      resources: [path.join(__dirname, '../node_modules/tinymce/skins/ui/oxide/fonts')],
    }),
  },
  'element-ui': {
    name: 'ELEMENT',
...
module.exports = {
  ...
  chainWebpackExternals(config) {
    ...
    const {
      ...
      importStyles,
      copyConfigs
    } = Object.keys(externalConfigs).reduce((result, key) => {
      const {
        scripts,
        styles,
        resources
      } = externalConfigs[key].resources(isDevelopment);
      const {
        version
      } = require(path.join(__dirname, `../node_modules/${key}/package.json`));
